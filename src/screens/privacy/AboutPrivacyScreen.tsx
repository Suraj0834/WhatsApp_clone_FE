...
export const AboutPrivacyScreen = () => {
  ...
  const loadPrivacySettings = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/users/privacy/about');
      setSelectedOption(response.data.aboutPrivacy || 'everyone');
      setExceptions(response.data.aboutExceptions || []);
    } catch (error) {
      console.error('Error loading about privacy:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const savePrivacySetting = async (option: PrivacyOption) => {
    try {
      setSaving(true);
      await axios.put('/users/privacy/about', {
        aboutPrivacy: option,
        aboutExceptions: option === 'contacts_except' ? exceptions : [],
      });
      setSelectedOption(option);
    } catch (error) {
      console.error('Error saving about privacy:', error);
    } finally {
      setSaving(false);
    }
  };
