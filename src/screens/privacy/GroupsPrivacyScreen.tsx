...
export const GroupsPrivacyScreen = () => {
  ...
  const loadPrivacySettings = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/users/privacy/groups');
      setSelectedOption(response.data.groupsPrivacy || 'everyone');
      setExceptions(response.data.groupsExceptions || []);
    } catch (error) {
      console.error('Error loading groups privacy:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const savePrivacySetting = async (option: PrivacyOption) => {
    try {
      setSaving(true);
      await axios.put('/users/privacy/groups', {
        groupsPrivacy: option,
        groupsExceptions: option === 'contacts_except' ? exceptions : [],
      });
      setSelectedOption(option);
    } catch (error) {
      console.error('Error saving groups privacy:', error);
    } finally {
      setSaving(false);
    }
  };
    ...
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subHeaderText}>
          You'll still be able to accept group invites through links or requests
        </Text>
      </View>
      ...
      <View style={styles.infoBox}>
        <Text style={styles.infoText}>
          When this setting is applied, you'll need to approve group invites from people who can't add you directly
        </Text>
      </View>
