...
export const LastSeenPrivacyScreen = () => {
  ...
  const loadPrivacySettings = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/users/privacy/last-seen');
      setSelectedOption(response.data.lastSeenPrivacy || 'everyone');
      setExceptions(response.data.lastSeenExceptions || []);
    } catch (error) {
      console.error('Error loading last seen privacy:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const savePrivacySetting = async (option: PrivacyOption) => {
    try {
      setSaving(true);
      await axios.put('/users/privacy/last-seen', {
        lastSeenPrivacy: option,
        lastSeenExceptions: option === 'contacts_except' ? exceptions : [],
      });
      setSelectedOption(option);
    } catch (error) {
      console.error('Error saving last seen privacy:', error);
    } finally {
      setSaving(false);
    }
  };
